...
- **`URL_PATTERN`** - set to "http" to make mailman generate links prefixed with "http" instead of default "https"
- **`SSL_FROM_CONTAINER`** - `d3fk/mailman2` can provide https connection (for other https possibilities see the advanced configuration section). In this case the `mod_ssl` is enabled and configured and a redirection from 80 to 443 in the container will be set automatically. The default value is set to "false" as a stringified boolean
